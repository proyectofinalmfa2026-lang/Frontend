    ...
    <div className="bg-[#0E0A2B] border border-[#22194A] rounded-xl p-6 flex flex-col items-center w-72 hover:border-[#C13A82]/40 transition-all duration-300 group">
      ...
      <div className="relative mb-4">
        {image ? (
          ...
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-[#C13A82] to-[#8C63C9] flex items-center justify-center text-2xl font-bold text-white border-2 border-[#22194A] group-hover:border-[#C13A82]/50 transition-colors">
            {initials}
          </div>
        )}
      ...
      <div className="text-center mb-3">
        ...
        {location && (
          <p className="text-[10px] text-[#5C5470] mt-1 flex items-center justify-center gap-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
            {location}
          </p>
        )}
      </div>
      ...
      {skills && skills.length > 0 && (
        <div className="flex flex-wrap justify-center gap-1.5 mb-4">
          {skills.map((skill) => (
            <span
              key={skill}
              className="text-[9px] px-2 py-0.5 rounded-full bg-[#22194A]/50 text-[#7B7497] border border-[#22194A]"
            >
              {skill}
            </span>
          ))}
      ...
      {githubStats && (
        <div className="flex items-center gap-4 mb-4 text-center">
          {githubStats.repos !== undefined && (
            <div>
              <p className="text-sm font-semibold text-[#D6D0DC]">{githubStats.repos}</p>
              <p className="text-[9px] text-[#5C5470]">Repos</p>
            </div>
          )}
          {githubStats.followers !== undefined && (
            <div>
              <p className="text-sm font-semibold text-[#D6D0DC]">{githubStats.followers}</p>
              <p className="text-[9px] text-[#5C5470]">Seguidores</p>
            </div>
          )}
        </div>
      )}
