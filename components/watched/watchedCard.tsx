...
export default function WatchedCard({ item }: WatchedCardProps) {
  ...
  const watchedDate = new Date(item.watchedAt).toLocaleDateString("es-PE", {
    day: "numeric",
    month: "short",
    year: "numeric",
  });
    ...
    <Link href={`/movies/${item.movie.id}`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
          ...
          <p className="text-xs text-[#3D3460] mt-1">Vista el {watchedDate}</p>
        </div>
